...
import { POSOrder, MenuItem, MenuCategory, OrderItem } from '@/types/restaurant';
import { 
  ArrowLeft, 
  Plus, 
  Minus, 
...
const DriveThruPOS: React.FC<DriveThruPOSProps> = ({ 
  categories,
  menuItems,
  onClose, 
  onSaveOrder,
  ...
  const OrderPanel = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className={`flex flex-col bg-card ${isMobile ? 'h-full' : 'border-l border-border'}`}>
      ...
      <div className="p-4 border-b border-border">
        <Label htmlFor="guestName" className="text-sm text-muted-foreground mb-2 block">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div className="flex items-center justify-between p-3 md:p-4 border-b border-border bg-card">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="text-base md:text-lg font-bold text-foreground">Order #{orderNumber}</div>
          </div>
        </div>
