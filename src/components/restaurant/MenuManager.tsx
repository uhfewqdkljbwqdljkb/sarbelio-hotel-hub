...
const MenuManager: React.FC<MenuManagerProps> = ({
  ...
  const handleDeleteCategory = (categoryId: string) => {
    ...
    onMenuItemsChange(menuItems.filter(i => i.categoryId !== categoryId));
  ...
  // Update category
  const handleUpdateCategory = (categoryId: string, name: string, icon: string) => {
  ...
  const handleDeleteItem = (itemId: string) => {
    onMenuItemsChange(menuItems.filter(i => i.id !== itemId));
  };

  // Update menu item
  const handleUpdateItem = (itemId: string, updates: Partial<MenuItem>) => {
    onMenuItemsChange(menuItems.map(i => 
          ...
          {categories.map(category => (
            <div
              ...
              className={`
                ...
                ${activeCategory === category.id ? 'bg-primary-100 text-primary-800' : 'hover:bg-secondary'}
              `}
